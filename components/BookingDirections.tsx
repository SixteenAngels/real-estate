...
export function BookingDirections({ booking, property, currentUser, onBack }: BookingDirectionsProps) {
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          getTravelInfo(property.coordinates)
            .then(setTravelInfo)
            .catch(error => {
    ...
    if (navigator.share && property) {
      try {
        await navigator.share({
          ...
          url: `https://maps.google.com/?q=${property.coordinates[0]},${property.coordinates[1]}`
        });
      } catch (error) {
        // Fallback to copying to clipboard
        const locationUrl = `https://maps.google.com/?q=${property.coordinates[0]},${property.coordinates[1]}`;
  ...
  const handleAddToCalendar = () => {
    ...
    const calendarUrl = `https://calendar.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(`${booking.type} - ${property.title}`)}&dates=${startDate.toISOString().replace(/[-:]/g, '').split('.')[0]}Z/${endDate.toISOString().replace(/[-:]/g, '').split('.')[0]}Z&details=${encodeURIComponent(`${booking.type} booking for ${property.title} at ${property.location}`)}&location=${encodeURIComponent(property.location)}`;
  ...
  const handleBookRide = () => {
    const uberUrl = `https://m.uber.com/ul/?action=setPickup&pickup=my_location&dropoff[latitude]=${property.coordinates[0]}&dropoff[longitude]=${property.coordinates[1]}&dropoff[nickname]=${encodeURIComponent(property.title)}`;
